...
baseline. These results do not establish broad world understanding. They
establish a falsifiable research program: if world processing is dominated by
persistent identity, local causal change, uncertainty, and branching, then state
should be treated as a primary computational substrate rather than as a
serialization target for tokens.
\end{abstract}
...
invariants. Second, it defines propagation as the central operation of
world-state processing and relates it to simplified local physical causality.
...
\node[box, fill=green!8] (statetitle) at (7.0,1.35) {State Processor};
\node[small] (cup) at (7.0,0) {cup};
\node[small] (hand) at (5.35,0) {hand};
\node[small] (table) at (8.65,0) {table};
\node[small] (edge) at (7.0,-1.15) {edge};
\draw[arrow] (hand) -- node[above, font=\scriptsize] {near} (cup);
\draw[arrow] (cup) -- node[above, font=\scriptsize] {on} (table);
\draw[arrow] (cup) -- node[right, font=\scriptsize] {near} (edge);
\node[align=center, font=\small] at (7.0,-2.15) {persistent entities\\patch / propagate};
\end{tikzpicture}
}
...
required next step, not as a solved problem.
...
\begin{tabular}{llll}
\toprule
Unit & Main workload & Strength & WPU contrast \\
\midrule
GPU & dense tensor kernels & throughput & graph-local state updates \\
TPU & matrix/tensor programs & systolic dense compute & sparse-dense routing \\
NPU & neural layer inference & efficient feedforward models & persistent state memory \\
LPU & token sequence inference & low-latency token generation & object/relation deltas \\
WPU & world-state processing & local update + branching & state-native primitive \\
\bottomrule
\end{tabular}
\caption{The WPU is best understood as a workload and architecture proposal, not
as a claim that current dense accelerators are obsolete.}
\label{tab:units}
\end{table}
...
but the present implementation remains a PyTorch reference model.
...
\draw[arrow] (d3) -- (log);
\end{tikzpicture}
\caption{Branch memory. A WPU shares the base state and stores branch-specific
deltas, reducing memory from \(O(BN)\) to \(O(N + B\Delta)\) when branches are
local.}
\label{fig:branch-memory}
\end{figure}
...
inspectable. They are not claimed to be optimal constants. The \(B\)-sweep and
training-step sweep below show that fixed thresholds can hurt accuracy, which is
why learned routing is a primary follow-up requirement.
...
\bottomrule
\end{tabular}
\caption{Routing sweep. The same local event routes differently depending on
the fraction of world state affected by the event.}
\label{tab:routing}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Baseline and ablation branch accuracy after 100 training steps. The
results are mixed: WPU variants are strong in small/intermediate regimes, while
the serialized-token baseline is competitive and strongest at \(N=84\). This
supports a regime hypothesis rather than universal WPU dominance.}
\label{tab:baselines}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Robust baseline comparison over five seeds, 150 training steps, and
256 evaluation samples per condition. The result supports a medium-local
regime advantage but refutes universal WPU quality dominance.}
\label{tab:robust-baselines}
\end{table}
...
\caption{Dense \(N\)-sweep change points over \(N=4,8,12,16,24,36,52,68,84,108,132,164,204,260\).
The v1 gap is now precise: accuracy advantage ends around \(N\approx120\),
while runtime advantage begins around \(N\approx124\) versus token baselines
and \(N\approx178\) versus dense graph.}
\label{tab:n-sweep-change}
\end{table}
...
\end{subfigure}
\caption{Dense \(N\)-sweep evidence. The scheduler transitions occur near the
theoretical \(\rho\) thresholds, but the accuracy crossover and runtime
crossover do not yet align.}
\label{fig:n-sweep}
\end{figure}
...
\caption{Dense \(B\)-sweep over \(B=1,\ldots,16\). Increasing branch pressure
moves routed WPU toward hybrid or dense execution, but accuracy is not
monotonic in \(B\). This is evidence that the fixed threshold scheduler should
be replaced by an accuracy-latency-aware learned router.}
\label{tab:b-sweep}
\end{table}
...
\caption{Dense training-step sweep at \(N=84, B=3\) over steps
\(0,5,10,15,20,30,40,50,75,100,125,150,200,250,300\). Many baselines peak
before 150 steps, so a single fixed training budget is not sufficient evidence
of model-family dominance.}
...
\toprule
Model & Acc. at 0 & Acc. at 128 & Acc. drop & MSE multiplier & Cup-MSE multiplier \\
\midrule
...
\caption{Controlled relation-noise stress test over irrelevant extra relation
edges \(0,4,8,16,32,64,128\). WPU-hybrid preserves branch accuracy best, while
the relation-aware Graph Transformer collapses under heavy irrelevant edge
noise. This supports frontier/routing control in noisy state graphs, not
universal WPU dominance.}
\label{tab:relation-noise}
\end{table}
...
\caption{Controlled affected-count stress test with strong background deltas.
The branch label remains cup-centric, so affected-background MSE is the relevant
failure-mode metric. Serialized token is best at the largest affected count,
while routed/sparse WPU remains competitive.}
\label{tab:affected-count}
\end{table}
...
\end{subfigure}
\caption{Controlled stress tests. Relation noise tests robustness to irrelevant
edges; affected count tests whether local-update models remain valid when the
changed region becomes less local.}
\label{fig:controlled-stress}
\end{figure}
...
A denser \(N\)-sweep sharpens this statement. With \(B=3\), routed execution
...
\(N\approx124\) and faster than dense graph around \(N\approx178\). The next
model target is therefore concrete: push the accuracy crossover beyond the
runtime crossover while preserving the sparse execution curve.
...
supports a quality dominance claim. A stronger paper must therefore compare
matched models on the full accuracy-compute-memory surface, not only on
accuracy.
...
The next stage requires stronger validation:
\begin{enumerate}[leftmargin=*]
\item balanced synthetic benchmarks with controllable \(N, \Delta N, k, h, B\);
\item ablations against sparse-only, dense-only, Graph Transformer, Set
Transformer, and serialized-token baselines;
\item controlled stress tests for irrelevant relations, global affected regions,
identity swaps, distractor objects, and long-horizon branch divergence;
\item learned routing compared with the hard \(\rho\) scheduler;
\item perception-to-state front-ends using slot/object discovery or supervised
segmentation;
\item checkpoint, rollback, and state-integrity mechanisms for persistent
delta updates;
\item calibration metrics for branch probabilities;
\item long-horizon rollout error and branch collapse analysis;
\item runtime and memory profiling on CPU/GPU;
\item real or simulator-backed object dynamics datasets such as learned-physics,
MuJoCo, Isaac Gym, or robotics manipulation benchmarks.
\end{enumerate}

A high-impact validation should not only report accuracy. It should report the
accuracy-compute-memory surface as the world size, affected fraction, causal
depth, and number of branches vary. The most important experiment is not whether
WPU wins on one small task; it is whether the predicted regime diagram appears:
state-native propagation should dominate in local-change regimes, dense models
should dominate in global-change regimes, and hybrid routing should occupy the
crossover.
...
routing, and branch deltas in a reproducible neural model.
