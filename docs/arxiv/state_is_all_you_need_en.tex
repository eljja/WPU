...
persistent, addressable objects with typed attributes, typed relations,
uncertainty, admissible deltas, and branch-local overlays. Objectification is
not a claim that perception is solved; it is the contract that defines the
...
physical laws. A controlled repair probe further shows that typed
objectification is not cosmetic: ungated geometry repair recovers frontier
recall but can attach distractors, whereas type-gated repair preserves precision
in the tested setting. A small learned relation scorer matches that type gate in
the same controlled distribution, but transfer to shifted or hidden-rule regimes
...
gates, and strict no-harm seed-stable gates do not solve the cross-seed
selection problem, and the candidate oracle remains substantially better. The
unresolved v2 problem is therefore invariant candidate descriptors,
risk-aware mechanism selection, and joint retriever-propagator training, not
merely the generation of more candidate sets.
