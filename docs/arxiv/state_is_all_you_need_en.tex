...
trained from candidate sets that minimize downstream branch loss, improves loss
over an interaction-teacher retriever across \(K=8,16,32\) and wins 14 of 15
seed/K conditions at \(N=2048\). These results do not establish broad world
understanding. They establish a falsifiable research program: if world
...
\section{V2 Retrieval: Regret-Distilled Working Sets}
...
working set that should be propagated. This is where explicit state creates an
additional intervention point. Before any tensor propagation step, the system
can select objects from the event frontier, score alternative working sets, and
train that selection policy against downstream state-prediction regret.
...
interaction selector is rarely the downstream oracle. We therefore evaluated a
regret-distilled retriever. On a validation split, the system evaluates base
...
The result remains bounded. It is same-seed validation-to-test distillation,
not a proof of robust cross-seed generalization. Diagnostic rerankers using
candidate entropy, maximum branch probability, and logit margin provide small
cross-seed improvements, and a train-only variant selector improves over a
static base selector across \(K=8,16,32\). However, these mechanisms do not
close the gap to the generated oracle. The unresolved v2 problem is therefore
invariant candidate scoring and joint retriever-propagator training, not merely
the generation of more candidate sets.
...
invariant candidate scoring across seeds and model instances; joint
retriever-propagator training; large-\(N\) sparse stability; regional dense
correction when pure sparse propagation loses global consistency; long-horizon
...
routing, branch deltas, and regret-aware working-set selection in a reproducible
neural model. The strongest current evidence is not universal dominance, but a
more precise mechanism: when world updates depend on a small, identifiable
causal working set inside a much larger state, explicit state enables both
sparse propagation and trainable object-level control before propagation.
