...
\(N_{\mathrm{bg}}=512\), total \(N=517\) baseline-complete micro-screen adds
WPU, graph, and token baselines at larger state size; the best WPU reaches
0.375000 versus the best baseline at 0.333333, but this is only coverage
evidence because it uses 3 seeds, 2 training steps, and 8 samples. The separate
higher-budget WPU-only \(N_{\mathrm{bg}}=512\) extension remains systems
feasibility evidence because the graph-transformer baseline did not complete
under the attempted protocol.
