...
worsens loss.
A PyBullet objectification-loss coupling audit adds a simulator-derived stress
test for the same boundary. In the current one-step corruption setting, branch
accuracy moves only weakly, but WPU sparse under heavy relation drop has the
largest MSE increase (0.087356), and selected-\(K\) deficit is the strongest MSE
predictor (\(|r|=0.481851\)). Thus objectification quality is now connected to
downstream degradation, but the coupling remains partial and should be extended
to closed-loop or multi-horizon rollouts.
...
This paper makes five contributions. First, it formulates a token-state
...
state objects and makes objectification quality measurable as a pre-propagation
contract. Third, it treats propagation over typed object relations as the state
analogue of attention and relates it to simplified local physical causality,
while explicitly separating this prior from claims of full physical
understanding. Fourth, it proposes a WPU architecture with event frontiers,
delta-state memory, branch overlays, and sparse, hybrid, and dense execution
paths selected by an affected-state crossover metric. Fifth, it provides a
minimal reference implementation, small-scale validation tasks, and a claim
ledger that records both positive regimes and failure boundaries. The goal is
to make a new execution substrate testable, not to present final evidence for
broad superiority.
...
final hardware evidence. A derived systems claim-boundary audit currently
separates four supported proxy axes from two partial trained axes and one
explicitly unmeasured real-power/sparse-kernel axis; it also shows that CUDA
peak-memory reduction is only 0.304080 despite strong latency proxy reductions.
A software runtime or middleware layer is the nearer
...
and train-selected closure is 0.093863. A joint object-set candidate gate,
which encodes each candidate working set before scoring it, is also negative:
best and safe closure are 0.101454, train-selected closure is 0.072167, and a
regression-heavy \(K=16\) ablation reaches only 0.034751. A fixed-candidate and
fixed-propagator downstream-loss selector trained on expected propagation loss
and no-harm mass is also negative: best closure is 0.106927, no deployment
satisfies harmful accept \(\leq 0.25\), and train-selected closure is 0.096833.
A learned candidate generator trained from downstream-regret object membership
creates additional oracle headroom, reaching 0.361251 closure at \(K=16\), but
the deployed evaluator recovers only 0.042951. A verified candidate controller
that appends label-free sparse/local-dense propagation signatures is also
weaker than direct candidate-regret gating: best closure is 0.024989, safe best
is 0.023029, and train-selected closure is 0.024989. A shallow candidate-aware
branch-logit propagation adapter also underperforms direct candidate-regret
gating: best/safe closure is 0.092185, and train-selected closure is 0.069911.
A joint utility verifier that combines candidate object-set tensors,
sparse/local-dense verification signatures, uncertainty, and no-harm safety is
also weaker than direct candidate-regret gating: best/safe closure is 0.097845,
and train-selected closure is 0.077781.
The unresolved v2 problem is therefore candidate generation and scoring learned
jointly with retrieval, verification, and propagation dynamics, not merely the
generation of more candidate sets, candidate-state features, post-hoc deployment
gates, verification-feature descriptors, a shallow selector-loss replacement,
fixed-propagator utility heads, or a small output adapter.
...
A follow-up calibration-statistic detector recovers the same safe policy using
base ECE and majority-prior gap rather than mechanism-name routing, with zero
nominal false adaptation; it is a stricter detect-and-adapt audit, but still
uses calibration labels and adaptation samples.
...
cost jointly.
A derived calibration-cost frontier audit makes this boundary explicit across
static gates, learned gates, mechanism-aware adaptation, and mechanism-selective
calibration routing. Global gates still fail, but the mechanism-selective audit
finds one non-reference calibration-safe policy under cost proxy $\leq 0.25$:
accuracy delta 0.029100, ECE delta -0.001652, Brier delta -0.030758, and cost
0.247355. This is a weak adapted positive sub-regime, not zero-shot
calibration-safe routing.
...
layer, not evidence that raw sparse dynamics are already stable. A
correction-trigger frontier sharpens the boundary: no tested trigger policy
meets integrity at least 0.8 with correction rate at most 0.25; the best
low-correction entropy gate reaches integrity 0.653668. A learned correction
trigger does not remove this boundary on the hard seed split: the best learned
trigger reaches integrity 0.958931 only at correction rate 0.791667, while the
best policy under correction rate at most 0.25 reaches integrity 0.523279. A
stable-transition loss sweep is a partial positive: the strongest delta-norm
configuration raises raw finite-clamped integrity to 0.633398, raises
selective-correction low-disruption score to 0.809071, and lowers correction
rate to 0.598333, but still yields no row satisfying both integrity at least
0.8 and correction rate at most 0.25.
A separate PyBullet coverage audit includes matched
\(N_{\mathrm{bg}}=256\), total \(N=261\) runs where WPU, graph, and token
baselines all complete. A medium-training run improves this evidence: the best
WPU reaches branch accuracy 0.466667 versus the best baseline at 0.450000, with
60.629526 times lower forward latency than that best-accuracy baseline.
...
5-seed \(N_{\mathrm{bg}}=512\), total \(N=517\) baseline-complete run adds
stronger large-state evidence: the best WPU reaches 0.433333 versus the best
baseline at 0.425000, with 57.595711 times lower forward latency than that
best-accuracy baseline. The WPU edge persists under a larger budget, but the
margin shrinks, so this remains conditional evidence.
\(N_{\mathrm{bg}}=512\) mechanism-diversity screens at the same total \(N=517\)
are a sharper claim-boundary result. The original screens were negative:
nominal-train evaluation over seven mechanisms gave WPU win/tie/loss 2/1/4 and
mean margin -0.047619, while multi-mechanism training gave 2/0/5 and mean margin
-0.095238. That audit exposed an input-contract defect: action-conditioned
event state and physical object-state scalars were present in the objectified
simulator state but omitted during tensorization. After preserving
\texttt{catch\_action}, \texttt{edge\_distance}, \texttt{hand\_distance},
\texttt{fall\_risk}, and \texttt{angular\_speed}, nominal-train shift recovers
to 4/0/3 with mean margin +0.002976, but multi-mechanism training remains
mixed/negative at 2/2/3 with mean margin -0.032738. Thus large explicit state
and small identifiable \(K\) are not enough; WPU also needs faithful
action/physical state encoding and learned or adapted mechanism-aware
propagation. The separate higher-budget WPU-only \(N_{\mathrm{bg}}=512\)
extension remains systems feasibility evidence because the graph-transformer
baseline did not complete under the attempted protocol. The later
\(N_{\mathrm{bg}}=4096\) relation-conditioned run extends this boundary to
total \(N=4101\), but only as WPU-only sparse feasibility evidence. It supports
the systems hypothesis that indexed state can avoid full recompute when the
selected working set remains tiny; it is not a baseline-complete accuracy
claim.
