...
- Baseline-complete large-`N` simulator superiority는 아직 지지되지 않는다.
  ...
  simulator superiority는 아니다. Higher-budget 5-seed N_bg=512, total `N=517`
  baseline-complete run은 더 강한 large-state evidence를 추가한다. Best WPU는
  ...
  유지되지만 margin은 줄어들기 때문에 conditional evidence로 남는다. 별도의
