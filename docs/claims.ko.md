...
`docs/experiments/world_copy_baseline_comparison_probe_results.ko.md`는 v3 world-copy
delta setting에서 같은 task로 WPU/token/graph/dense를 비교한 첫 screen이다. WPU는 max
selected `K=16`, mean work proxy `8.789551`, mean bytes proxy `316.423828`을 유지한다.
Dense graph와 serialized token baseline은 mean work proxy `2727.406250`을 사용하고,
quadratic graph-transformer proxy는 `17935805.573893`을 사용한다. 중요한 negative
result도 있다. Dense graph와 serialized token의 raw delta MSE(`0.003778`,
`0.004533`)가 WPU(`0.020818`)보다 낮다. 따라서 이 결과는 낮은 work/memory와
accuracy-per-work를 지지하지만 raw accuracy dominance는 아니다.
