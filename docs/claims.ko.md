...
| C6 | large `N`은 causal working set `K`가 작고 tensorization 전에 식별될 때만 WPU에 유리하다. | 조건부 systems hypothesis로 초기 증거가 있음. | `docs/experiments/cws_balanced_branch_8m_gpu_event_conditioned_final_results.md`, `docs/experiments/wpu_v2_pre_tensor_indexed_n_sweep_results.md`, `docs/experiments/large_n_target_frontier_pooling_probe_results.ko.md`, `docs/experiments/world_copy_index_probe_results.ko.md`, `docs/experiments/world_copy_causal_index_stress_results.ko.md`, `docs/experiments/world_copy_escalation_correction_probe_results.ko.md`, `docs/experiments/world_copy_learned_correction_probe_results.ko.md`, `docs/experiments/world_copy_baseline_comparison_probe_results.ko.md`, `docs/experiments/world_copy_streaming_region_guard_probe_results.ko.md`, `docs/experiments/world_copy_dual_index_escalation_probe_results.ko.md`, `docs/experiments/world_copy_uncertainty_observation_policy_probe_results.ko.md`, `docs/experiments/world_copy_adaptive_observation_budget_probe_results.ko.md`, `docs/experiments/world_copy_learned_observation_policy_probe_results.ko.md`, `docs/experiments/world_copy_calibrated_observation_policy_probe_results.ko.md`, `docs/experiments/world_copy_online_calibration_policy_probe_results.ko.md`, `docs/experiments/pybullet_shift_generalization_n1024_mechanism_relation_results.md`, `docs/experiments/pybullet_shift_generalization_n2048_mechanism_relation_results.md`, `docs/experiments/pybullet_shift_generalization_n4096_mechanism_relation_results.md`, `docs/versions/wpu_v1_closure.md`. | `N`이 큰 것만으로는 충분하지 않다. retrieval이 `O(N)`에 가깝거나 causal state를 놓치면 실패한다. 업데이트된 v3 baseline-comparison과 streaming screen은 bounded reliable local region 조건에서만 positive다. Dual-index, uncertainty-observation, adaptive-budget, learned-policy, calibrated-policy, online-calibration probe는 조건을 더 좁힌다. 누락된 causal object가 bounded correction/observation으로 접근 가능하면 WPU는 sublinear하게 복구할 수 있고 clean observation-budget rule은 학습 가능하며, 작은 labeled calibration set과 online hit/miss feedback은 anomaly-shift 실패를 줄일 수 있다. Paired verified-online probe는 value-gated bounded top-up budget을 유지하면서 learned/online correction을 개선한다. Naive base-budget trimming은 negative지만, oracle-free sequential hit/miss stopping은 `N=8192` noisy over-observation objective를 `0.193618`에서 `0.181400`으로 개선하고 budget을 줄이면서 recall을 유지해 labeled calibration `0.180837`에 근접한다. Inference-time clean prior가 없는 learned safety-calibrated composition gate는 clean을 verified 수준인 `0.159201`까지, weak anomaly를 verified 수준인 `0.201223`까지 복구하고 work를 `K≈32` 근처로 bounded하게 유지한다. 남은 문제는 noisy strict no-harm이다. noisy objective는 sequential에 가깝지만(`0.181323` versus `0.181089`) 아직 작은 추가 observation cost를 지불한다. Dense/external observation은 raw accuracy 상한으로 남는다. |
...
최신 C6/P2 v3 learned-correction 업데이트:
`docs/experiments/world_copy_learned_correction_probe_results.ko.md`는 회복된 local
correction candidate가 learned delta update를 개선하는지 테스트한다. 작은
relation/state-conditioned MLP를 local candidate 위에서 학습한다. True relation
confidence가 `0.2`일 때 `sparse_confident_relations`는 mean delta MSE `0.275312`를
남기지만, `hybrid_escalation_region`은 mean delta MSE를 `0.006365`까지 낮추고 max
selected `K=16`, max touch ratio `0.24489796`을 유지한다. 이는 learned local
correction에 대한 controlled substrate positive이지, token/graph baseline victory나
long-horizon world-copy result는 아니다.

최신 C6/P2 v3 baseline-comparison 업데이트:
`docs/experiments/world_copy_baseline_comparison_probe_results.ko.md`는 bounded local-region
guard를 포함한다. Guard는 max selected `K=16`, mean work proxy `9.333333`, mean bytes
proxy `336.000000`을 유지하면서 raw delta MSE `0.002646`을 달성한다. 이는 controlled
same-task screen에서 dense graph `0.003810`, serialized token `0.003223`보다 낮다.
중요한 negative result도 있다. `wpu-hybrid-context`는 MSE `0.020904`이므로 shallow
context concatenation은 gap을 해결하지 못한다. 따라서 positive claim은 구체적이다.
Bounded reliable local region은 full-state recompute 없이 missing-relation gap을 닫을 수
있다.

최신 C6/P3/P4 v3 streaming 업데이트:
`docs/experiments/world_copy_streaming_region_guard_probe_results.ko.md`는 region-guard
결과를 object churn과 region migration을 포함한 H=25 controlled stream으로 확장한다.
`wpu-region-guard`는 max selected `K=8`, trajectory MSE `0.000000`, integrity
`1.000000`, correction cost `0.000000`, work proxy `8.000000`, bytes proxy
`288.000000`을 유지한다. Dense state copy도 integrity는 같지만 `N`과 함께 증가하는
full-state work/bytes를 사용한다. Relation-frontier-only WPU는 약 `0.416667` 수준의
correction rate가 필요하다. 이는 controlled oracle-law evidence이지 실제 simulator 또는
learned-transition evidence는 아니다.
