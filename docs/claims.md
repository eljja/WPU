...
| C7 | Explicit state exposes pre-propagation working-set control. | Supported as the strongest current v2 mechanism claim, but the dashboard still records partial/failing gaps. | `docs/experiments/wpu_v2_retriever_regret_distillation_results.md`, `docs/experiments/wpu_v2_invariant_set_scorer_results.md`, `docs/experiments/wpu_v2_candidate_oracle_gap_decomposition.md`, `docs/experiments/wpu_v2_candidate_noharm_gate_results.md`, `docs/experiments/wpu_v2_priority_dashboard.md`. | Candidate oracle remains better; aggregate policy selection and margin-only sample-level no-harm gates do not close the gap; cross-seed robust scoring and long-horizon model-delta stability are not solved. |
