...
`docs/experiments/world_copy_baseline_comparison_probe_results.md` is the first
same-task WPU/token/graph/dense comparison screen for the v3 world-copy delta
setting. WPU keeps max selected `K=16`, mean work proxy `8.789551`, and mean
bytes proxy `316.423828`. Dense graph and serialized token baselines use mean
work proxy `2727.406250`, while the quadratic graph-transformer proxy uses
`17935805.573893`. The negative result is important: dense graph and serialized
token have lower raw delta MSE (`0.003778` and `0.004533`) than WPU (`0.020818`).
Thus this supports lower work/memory and accuracy-per-work, not raw accuracy
dominance.
