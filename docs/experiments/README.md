...
- `world_copy_online_calibration_policy_probe_results.md` and
  ...
  `noisy_anomaly` objective from learned `0.266230` to `0.193618` and
  `weak_anomaly` from `0.334783` to `0.202765`, while keeping selected work near
  `32`. It also improves the clean paired stream from learned `0.166575` to
  `0.159478`, approaching hand adaptive `0.154890`. The verifier spends bounded
  ...
  `0.193618` to `0.181400`, nearly matching labeled calibration `0.180837`.
  `wpu-composed-online-observation` selects sequential stopping unless online
  ...
  from paired sequential/verified outcomes and adds a conservative clean-recovery
  prior. It recovers clean to `0.159211`, preserves noisy no-harm at `0.181089`,
  and improves weak anomaly to `0.194131`, better than verified `0.200840`.
  The remaining failure is replacing the hand-coded clean prior with a learned
  safety-calibrated gate.
...
- `pybullet_cup_benchmark_n512_baseline_micro_results.md` and
  ...
  superiority result.
- `pybullet_cup_benchmark_n512_medium_results.md` and
  `pybullet_cup_benchmark_n512_medium_results.ko.md`: 5-seed N_bg=512 matched
  medium benchmark. Best WPU accuracy is `0.387500` versus best baseline
...
- `pybullet_shift_generalization_n512_branch_expert_results.md`
  ...
  propagation.
- `pybullet_shift_generalization_n512_mechanism_relation_results.md`
...
- `pybullet_shift_generalization_n1024_mechanism_relation_results.md`
  ...
  total `N=1029`, the 5-seed expansion reaches macro accuracy `0.639286` versus
...
- `pybullet_relation_closed_loop_rollout_n512_integrity_results.md`
  ...
  selected `K = 4.354167` but collapses at H=25 with integrity `0.250368`,
  trajectory MSE `6.975125`, and branch accuracy `0.208333`. Finite projection
  raises H=25 integrity to `0.876760` but has worse prediction quality than the
  bounded transition (`1.695024` trajectory MSE and `0.250000` branch
  accuracy). Earlier scalar regularization, temporal scaling, short-stride
  targets, and explicit multi-horizon targets were negative. The first
  raw-model positive result
  is bounded delta parameterization inside the transition head: H=25 integrity
  reaches `0.611900` at bound `0.5`, `0.652870` at `0.25`, `0.793182` at
  `0.1`, and `0.870264` at `0.05`, while selected `K` remains `4.354167` and
  no correction, rollback, rejection, or dense fallback is used. The same
  ...
  transition head is the first small positive follow-up. In the preferred
  5-seed matched comparison, H=25 branch accuracy improves from `0.612500` to
