...
- Generated report: `artifacts/causal_working_set_8m_gpu/results.md`
- Figures: generated by `scripts/plot_causal_working_set.py`

Do not promote the generated report into `docs/experiments/` until the run has
been checked for seed completeness, parameter matching, and the claim boundary
in `docs/claims.md`.

Generate figures:

```powershell
python scripts/plot_causal_working_set.py `
  --input artifacts/causal_working_set_8m_gpu/n-sweep.csv `
