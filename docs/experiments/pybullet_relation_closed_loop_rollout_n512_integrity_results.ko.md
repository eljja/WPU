...
| `relation_bounded_delta005_5seed` | 0.868758 | 1.063341 | 544.710934 | 0.612500 |
...
5-seed matched validation에서도 방향은 유지되지만 더 약하다. Relation bounded baseline은 H=25
branch accuracy `0.612500`, trajectory MSE `1.063341`, target-object MSE `544.710934`이고,
mechanism-target은 branch accuracy `0.675000`, trajectory MSE `1.061493`, target-object MSE
`543.834596`이다. 그러나 state-integrity는 `0.868758`에서 `0.850574`로 낮아진다. 따라서
target-head는 유망하지만 state-validity tradeoff가 있다.

이번 constrained target-head follow-up은 standalone 해결책이 아니다. 새
`wpu-cws-indexed-mechanism-target-constrained`는 target residual을 position/velocity channel로
제한하지만, H=25에서 branch accuracy `0.650000`, trajectory MSE `1.063031`,
