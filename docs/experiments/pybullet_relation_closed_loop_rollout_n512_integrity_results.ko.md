...
target-head는 유망하지만 state-validity tradeoff가 있으며, 다음 단계는 target-object improvement와
integrity를 동시에 보존하는 constrained target-head dynamics다.

## 다음 조치

- target-object trajectory MSE를 position과 velocity로 계속 분리해 보고한다.
- target-object improvement와 state-integrity를 동시에 보존하는 constrained target-head를 설계한다.
