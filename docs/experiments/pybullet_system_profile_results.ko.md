...
- Metric:
  ...
  - `sparse_peak_memory_reduction`: CUDA profile에서 같은 random untrained
    forward proxy의 peak allocated-memory reduction.
...
CUDA profile은 latency 측면의 주장을 강화한다. `N≈2052.4`에서 random full-state
`graph-transformer` forward는 약 `579 ms`, selected-state sparse WPU forward는 약
`2.2 ms`이며 sparse forward latency reduction은 `0.996216`이다. Selected local-dense
WPU forward는 약 `3.24 ms`이고 reduction은 `0.994417`이다. 반면 peak allocated CUDA
memory reduction은 sparse 기준 `0.304080`에 그친다. 따라서 allocator와 model-parameter
memory는 forward latency만큼 줄어들지 않는다.

하지만 이것은 lower power나 matched-accuracy speedup을 증명하지 않는다. 아직 random
untrained model measurement이며 energy, trained accuracy parity, custom sparse kernel,
full GPU occupancy analysis를 포함하지 않는다. 방어 가능한 주장은 더 좁다.
...
- Profiler는 이제 CPU tensorization latency, random untrained CPU forward proxy,
  random untrained CUDA forward/peak-memory proxy를 측정하지만,
  trained matched-accuracy latency, CUDA allocator traffic, energy는 아직 없다.
...
- 같은 `N` setting에서 trained matched-accuracy forward latency, CUDA allocator
  traffic, energy를 추가한다.
