...
proxy이며 irregular sparse-kernel overhead, cache behavior, GPU occupancy, 실제
memory traffic을 포함하지 않는다. 방어 가능한 주장은 더 좁다.
...
- Profiler는 proxy byte와 proxy work를 측정하며 wall-clock latency 또는 energy를
  측정하지 않는다.
...
- 같은 `N` setting에서 matched model forward pass의 runtime latency와 CUDA memory를
  추가한다.
