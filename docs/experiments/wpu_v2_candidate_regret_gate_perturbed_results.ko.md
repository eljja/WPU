...
최고 closure는 `0.339525` (`K=16`, `candidate_regret_gate_m0p0025`)다. P1 목표 `0.5`를 기준으로 candidate-regret deployment가 candidate-oracle gap을 충분히 닫는지와 harmful accept를 억제하는지를 동시에 본다. Harmful accept <= `0.25` 조건의 conservative best는 `0.329756` (`candidate_regret_gate_m0p005`)다. Train-selected deployed best는 `0.312586` (`K=16`)다.
