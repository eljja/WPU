...
- P2: Selective correction으로 수정 범위는 줄였지만 correction trigger rate는 줄이지 못했다. 다음은 transition model 자체를 안정화하고, correction trigger를 learned uncertainty/state-validity objective로 학습해 대부분의 sparse update를 수정하지 않아도 integrity가 유지되게 만든다.
