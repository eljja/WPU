...
- P2: 단순 delta-norm, rollout-consistency, state-validity regularization은 부족하다. Guarded/corrected state-store projection은 유지하되 correction rate를 낮추는 learned correction과 uncertainty escalation을 모델-메모리 계층에 넣는다.
...
- P5: Hand-threshold recompute가 아니라 학습 가능한 저비용 uncertainty gate, branch calibration loss, multi-step ECE/Brier/NLL을 추가한다.
