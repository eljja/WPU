...
Public API도 이 단계를 명시적으로 표현한다. `LocalLawHypothesis`는 candidate local
rule의 relation type, expression, input field, parameter, evidence, status를 기록한다.
`evaluate_law_revision`은 base/revised error, relative improvement, accept/collect-more
decision, optional relation-selection gap과 law-residual gap을 담은 `LawRevisionReport`를
반환한다. 이것은 반증 가능한 theory revision을 위한 reporting contract이지 full physics
engine이 아니다.
