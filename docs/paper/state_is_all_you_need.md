...
Adding candidate object-set tensors, sparse/local-dense verification
signatures, uncertainty, and no-harm safety in a joint utility verifier is also
insufficient: best/safe closure is `0.097845`, and train-selected closure is
`0.077781`.
...
fixed-propagator utility head or small output adapter.
...
- Robust branch-prior adaptation under mechanism shift. The PyBullet
  ...
  detect-and-adapt evidence, not zero-shot evidence. A follow-up
  calibration-statistic detector recovers the same safe policy using base ECE
  and majority-prior gap rather than mechanism-name routing, with nominal false
  adaptation `0`; it is a stricter detect-and-adapt audit, but still uses
  calibration labels and adaptation samples. A separate 7-seed
  ...
  static gates, learned gates, mechanism-aware adaptation, and
  mechanism-selective calibration routing. Global gates still fail, but the
  mechanism-selective audit finds `1` non-reference calibration-safe policy
  under `cost_proxy <= 0.25`: accuracy delta `0.029100`, ECE delta
  `-0.001652`, Brier delta `-0.030758`, and cost `0.247355`. This is a weak
  adapted positive sub-regime, not zero-shot calibration-safe routing.
- Baseline-complete large-`N` simulator superiority. The PyBullet coverage
  ...
  positive P3 evidence rather than strong simulator superiority. A low-training
  N_bg=512, total `N=517` baseline-complete micro-screen adds WPU, graph, and
  token baselines at larger state size; best WPU reaches `0.375000` versus best
  baseline `0.333333`, but this is only coverage evidence because it uses 3
  seeds, 2 training steps, and 8 samples. The separate higher-budget WPU-only
  N_bg=512 extension remains systems feasibility evidence because the
