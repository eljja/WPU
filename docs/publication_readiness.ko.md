...
병목을 줄이지 못했다. 따라서 다음 단계는 target set이나 scalar bound만 바꾸거나
unrolled loss만 추가하는 것이 아니라 branch-conditioned local transition dynamics와 더 강한
target-object transition head를 학습하는 방향이어야 한다.
