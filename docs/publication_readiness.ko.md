...
Compact thesis, 신규성, evidence boundary는 `docs/research_thesis.ko.md`를 함께 본다.
...
외부에 공표 가능한 신규성은 execution model에 있다. WPU는 objectified state를 계산
기판으로 두고, object identity, relation traversal, causal working-set selection,
delta overlay, branch revision을 native operation으로 만든다. 이는 sequence-only
processing과 구분되는 창의적이고 반증 가능한 방향이지만, 아직 universal superiority가
아니라 regime hypothesis다.
...
v3 world-copy index 확장은 large-`N` 논리를 개선하지만 완성하지는 않는다.
`WorldCausalIndex`는 controlled non-causal-background probe에서 total `N=10004`까지
selected causal slice를 `K = 4`로 유지할 수 있음을 보인다. 이는 pre-tensor causal
retrieval을 위한 substrate evidence로는 공표 가능하다. 그러나 trained real-world
modeling, noisy perception-to-state construction, long-horizon physical prediction을
해결했다는 증거는 아니다.
...
| route-state contract가 너무 압축되어 있었음 | Route decision이 mechanism-aware가 되려면 objectified action/physical state가 다른 곳에만 있고 route head에는 압축 geometry만 전달되는 구조면 안 된다. | `docs/experiments/wpu_v2_route_physics_contract_smoke_results.ko.md`는 physics/state regret head가 이제 pair geometry, target physical scalar, selected-set physical scalar, `force`, `catch_action`을 받음을 검증한다. Smoke run은 성능 증거가 아니지만, 후속 P1/P4 실험 전에 구조적 mismatch를 제거한다. | 확장된 route-state context로 full staged-regret 및 mechanism-shift sweep을 다시 수행하고 route-regret correlation, dense-compute rate, harmful accept, shifted accuracy/calibration을 비교해야 한다. |
| cross-seed/cross-task transfer가 불완전함 | synthetic gain은 generator artifact에 overfit될 수 있다. | 여러 cross-seed reranker/gate가 실패하거나 부분 개선에 그쳤다. 7-seed PyBullet benchmark는 seed fragility를 줄였고 `N=133`에서 WPU sparse branch accuracy가 `0.547619`로 serialized-token `0.539683`보다 약간 높지만, serialized-token이 여전히 더 빠르다. 3-seed leave-family-out probe는 WPU win-rate `0.750000`을 보이나 held-out `catch_heavy` branch-prior shift에서는 실패한다. 7-seed branch-prior audit은 이 원인을 확인한다. `catch_heavy` majority prior accuracy는 `0.753968`이고 best WPU는 `0.408730`이다. 7-seed mechanism-prior adaptation probe는 shifted WPU win-rate를 `0.333333`에서 `0.666667`로 올리지만, full shift generalization에는 도달하지 못한다. Prior-strength sweep도 best shifted win-rate는 `0.666667`에 머문다. Accuracy-best 설정은 `strength=0.75`, mean WPU accuracy `0.601852`다. Calibration-selected prior strength는 mean accuracy/ECE를 개선하지만 shifted WPU-vs-baseline win-rate는 `0.333333`에 머문다. Few-shot mechanism adaptation은 adapted protocol에서 긍정적이다. Shifted WPU win-rate는 `1.000000`, mean WPU-baseline margin 변화는 `0.050264`지만 mechanism-specific calibration sample을 사용한다. Mechanism-aware adaptive policy는 adapted regime을 더 강화한다. Shifted WPU win-rate는 `1.000000`, mean accuracy 변화는 `0.198412`, margin 변화는 `0.058201`, ECE 변화는 `-0.099347`, Brier 변화는 `-0.155443`이다. Calibration-statistic shift detector는 base ECE와 majority-prior gap으로 같은 safe policy를 복원하고 nominal false adaptation `0`을 달성해 mechanism-name oracle 의존을 줄인다. 이는 detect-and-adapt evidence이지 zero-shot evidence는 아니다. 7-seed composition-shift stress는 WPU에 accuracy-positive 결과를 보인다(win-rate `1.000000`, mean delta `0.071428`) 하지만 calibration 약점은 남아 있다. N_bg=512 mechanism-diversity audit은 mixed다. 원본 screen은 nominal-train `2/1/4`, margin `-0.047619`, multi-mechanism `2/0/5`, margin `-0.095238`로 negative였고, large N과 작은 K만으로 mechanism generalization이 되지 않음을 보였다. Action/physical state scalar를 tensorization에서 보존한 뒤 nominal-train screen은 `4/0/3`, margin `+0.002976`으로 회복됐지만 multi-mechanism screen은 `2/2/3`, margin `-0.032738`로 여전히 mixed/negative다. | 더 큰 seed sweep, 새 synthetic generator, 더 어려운 simulator mechanism, leave-generator-family-out validation, mechanism-aware propagation, 명시적 mechanism-shift detector, selective adaptation policy. |
| long-horizon state integrity가 부분 해결됨 | persistent state는 delta overlay가 장기적으로 망가지지 않을 때만 장점이다. | PyBullet state-integrity audit은 이제 constraint validity, bounded delta drift, branch stability, correction/rollback, simulator-resynchronized trajectory MSE, final branch accuracy를 함께 추적한다. 이전 memory guard 실험은 projection/rollback이 applied state를 보호할 수 있음을 보였지만 learned dynamics 증거는 아니었다. Relation-conditioned rollout audit은 첫 raw-transition positive다. Bounded delta `0.05`는 H=25 integrity `0.870264`를 correction, rollback, rejection, dense fallback 없이 달성한다. 또한 trajectory MSE `0.707117`, branch accuracy `0.729167`로 finite projection의 trajectory MSE `1.695024`, branch accuracy `0.250000`보다 낫다. 남은 gap은 target-object trajectory MSE `361.358309`가 여전히 크다는 점이다. Learned adaptive bounds, split position/velocity bounds, target-object delta loss, stabilized truncated unroll도 이 병목을 줄이지 못했다. Branch-weighted target-local transition head는 첫 작은 개선을 만든다. 선호하는 5-seed matched comparison에서 H=25 branch accuracy는 `0.612500`에서 `0.675000`으로, target-object MSE는 `544.710934`에서 `543.834596`으로 개선되지만, integrity는 `0.868758`에서 `0.850574`로 낮아진다. Channel-masked constrained target-head follow-up은 negative다. H=25 branch accuracy는 `0.650000`, target-object MSE는 `544.588713`, integrity는 `0.846955`에 그친다. 따라서 이는 usable sparse rollout baseline이자 유망한 transition-head 방향이지 high-fidelity learned physics는 아니다. | state-validity-aware transition learning, relation-conditioned rollout message, calibrated uncertainty escalation, long-horizon branch/trajectory evaluation. |
| real-world 또는 simulator-backed grounding이 아직 좁음 | world processing claim은 toy object physics 밖의 증거가 필요하다. | 현재 evidence는 synthetic robot-cup/CWS data와 PyBullet state를 포함한다. Simulator coverage audit는 breadth와 superiority claim을 분리한다. 본훈련 baseline-complete cup evidence는 7 seeds와 `N=133`까지이고, matched `N=261` evidence는 저훈련 screen과 medium-training baseline-complete run을 모두 포함한다. N=261 medium run에서 best WPU accuracy는 `0.466667`, best baseline은 `0.450000`이며, best WPU는 해당 best-accuracy baseline보다 forward latency 기준 `60.629526x` 빠르다. Matched `N=517` evidence는 이제 저훈련 micro-screen, 5-seed medium run, higher-budget 5-seed run을 포함한다. N=517 higher-budget run에서 best WPU accuracy는 `0.433333`, best baseline은 `0.425000`이고, best WPU는 해당 best-accuracy baseline보다 forward latency 기준 `57.595711x` 빠르다. 이는 P3 evidence를 강화하지만 margin이 줄어들고 단일 cup-family one-step benchmark에 머문다. Large-state mechanism coverage는 total `N=517`에서 7개 mechanism까지 확장됐다. Action/physical state 보존 후 nominal-train shift는 회복됐지만 multi-mechanism law-learning gap은 남아 있으므로, shift evidence는 simulator superiority를 닫은 것이 아니라 mechanism-aware propagation 필요성을 드러낸다. Rollout diagnostic은 horizon 25, objectification-quality evidence는 7개 corruption setting, systems profile은 `N≈2052`까지 포함한다. N_bg=4096 mechanism-relation run은 total `N=4101`까지 도달하지만 WPU-only sparse feasibility evidence이며, 같은 protocol의 matched dense/token/graph baseline은 완료되지 않았다. Simulator family는 여전히 좁고 대부분 single-object manipulation에 가깝다. | MuJoCo/Isaac/robotics/game-server/digital-twin benchmark, explicit state extraction, perception-to-state adapter, mechanism-aware propagation, baseline-complete large-N comparison. |
...
| hierarchical world-copy maintenance는 아직 substrate 수준임 | 실제 world copy에는 streaming update, region migration, stale-state eviction, noisy relation 아래의 causal-index recall이 필요하다. | `WorldCausalIndex`는 controlled probe에서 total `N=10004`까지 `K=4`를 유지하고 non-causal selection `0`을 기록한다. | object creation/deletion, region movement, noisy/missing relation, causal slice recall/precision, downstream trained propagation accuracy를 포함한 streaming world-copy benchmark. |
...
첫 larger step/sample stress audit은 accuracy 관점에서 negative다. 명시적 training-pool
control을 추가한 뒤 h32 WPU는 `0.534524`, best baseline은 `0.598810`이고, 공정한 h64
check에서도 WPU는 `0.603571`, serialized-token은 `0.622619`이다. WPU dense compute는
계속 `0.000000`이므로 efficiency story는 유지되지만, accuracy story는 현재 head를
그대로 scale하는 것이 아니라 sparse transition-head expressivity를 개선해야 한다.
첫 expressivity attempt도 negative다. Branch-specific output expert는 h32 stress
...
`wpu-cws-indexed-mechanism-relation` route는 5-seed h32 stress macro accuracy
`0.639286`을 달성했고 graph-transformer는 `0.597143`이며 dense compute는
...
필요하다.
새 relation closed-loop rollout diagnostic은 과장을 막는다. Raw relation WPU의 H=25
integrity는 `0.250368`, trajectory MSE는 `6.975125`, branch accuracy는 `0.208333`에
그친다. Finite projection은 integrity를 `0.876760`까지 올리지만 trajectory MSE
`1.695024`, branch accuracy `0.250000`으로 예측 성능은 약하다. Bounded delta
parameterization은 첫 transition-level positive이며 usable sparse rollout baseline을
만들지만 target-object trajectory error는 여전히 크다. Learned adaptive bounds,
position/velocity split bounds, target-object delta loss, stabilized truncated unroll도 이
병목을 줄이지 못했다. 새 branch-weighted target-local transition head는 작은 positive step이다.
5-seed H=25 branch accuracy는 `0.612500`에서 `0.675000`으로, target-object MSE는
`544.710934`에서 `543.834596`으로 개선되지만 integrity는 `0.868758`에서 `0.850574`로
낮아진다. 단순 channel mask는 target-head 이득을 보존하지 못했으므로 다음 단계는
state-validity-aware target-object dynamics다.
