...
position/velocity split bounds, target-object delta loss도 이 병목을 줄이지 못했다. 따라서
다음 단계는 target set이나 scalar bound만 바꾸는 것이 아니라 unrolled
branch/trajectory-consistent objective를 학습하는 방향이어야 한다.
