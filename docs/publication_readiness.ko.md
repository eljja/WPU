...
| unknown-theory discovery는 장기 연구 프로그램일 뿐임 | 아직 모르는 규칙성을 드러내는 learned relation은 알려진 relation 사용보다 훨씬 강한 주장이다. | 현재 두 개의 synthetic positive result가 있다. Object-history hidden-mechanism probe는 renamed type에서 relation precision/recall `0.987500`을 기록한다. Local-law probe는 history-derived relation selector와 해석 가능한 inverse-distance law를 held-out `hidden_inverse`로 transfer하여 delta MSE `0.000828`을 기록하며, no-relation 또는 type prior는 `0.445909`에 머문다. | simulator-backed held-out rule 또는 hidden-mechanism benchmark에서 learned object relation이 prediction을 개선하고 반증 가능한 새 구조를 제시하는 증거. |
