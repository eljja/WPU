...
만들지만 target-object trajectory error는 여전히 크다. 따라서 다음 단계는 target set만
바꾸는 것이 아니라 adaptive per-feature/per-relation transition bound를 개선하는 방향이어야
한다.
