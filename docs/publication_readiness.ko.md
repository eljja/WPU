...
대한 일반 tensor processing으로 약해진다.
...
| 객체화 품질 benchmark가 없음 | WPU 성능은 올바른 identity, relation, delta construction에 의존한다. | 현재 실험은 synthetic object state가 주어지는 조건이 많다. | missed object, identity swap, relation error, downstream propagation loss를 측정하는 object construction benchmark. |
...
7. identity stability, relation precision/recall, delta locality, objectification mistake로 인한 downstream error를 측정한다.
