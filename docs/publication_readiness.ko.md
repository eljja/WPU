...
| relation repair가 false hypothesis를 추가할 수 있음 | Repair는 누락된 local connectivity를 복구할 수 있지만, spurious edge는 `K`를 키우고 sparse precision을 낮출 수 있다. | relation-repair probe는 near distractor가 있을 때 ungated repair가 frontier recall은 복구하지만 precision이 `0.078994`로 떨어지고, type-gated 및 learned-scorer repair는 controlled case에서 precision을 `1.000000`으로 회복함을 보인다. | simulator relation 대비 repair precision/recall, repair 전후 downstream loss, distribution shift에서 harmful repaired edge를 reject하는 learned gate. |
