...
| unknown-theory discovery는 장기 연구 프로그램일 뿐임 | 아직 모르는 규칙성을 드러내는 learned relation은 알려진 relation 사용보다 훨씬 강한 주장이다. | Object-history hidden-mechanism probe는 첫 synthetic positive result를 제공한다. `contact_transfer`와 `support_transfer`로 학습한 history scorer가 held-out `hidden_field`에서 relation precision/recall `0.988281`, downstream accuracy `0.996094`를 기록한다. | simulator-backed held-out rule 또는 hidden-mechanism benchmark에서 learned object relation이 prediction을 개선하고 반증 가능한 새 구조를 제시하는 증거. |
