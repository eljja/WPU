...
2. long-horizon state integrity를 단순히 보고하는 것을 넘어 개선한다. 단순
   ...
   부족하다. Finite-corrected projection은 rollback이나 dense escalation 없이 applied
   state를 보호하지만 correction이 너무 자주 발생하므로 correction layer,
   uncertainty escalation, 낮은 correction frequency가 필요하다.
...
5. Calibration을 단순 보고가 아니라 개선한다. Static uncertainty-gated recompute는
   거의 full local-dense recompute일 때만 유의미하게 개선된다. 학습 가능한 저비용
   uncertainty gate, temperature/calibration head, multi-step ECE/Brier/NLL을 추가한다.
