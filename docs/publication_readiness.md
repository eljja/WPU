...
2. Improve long-horizon state integrity, not only report it. Simple delta-norm,
   ...
   is a small positive step: H=25 target-object MSE improves to `357.220733`
   and branch accuracy to `0.750000`. The next step is larger-seed validation
   and stronger target-object transition dynamics.
