...
| Hardware claims are unsupported | WPU as a processing unit requires systems evidence, not only PyTorch models. | A PyBullet systems profile now separates full-state tensorization, indexed WPU tensorization, sparse work proxy, branch-overlay memory proxy, and CPU tensorization latency. At `N≈2052.6`, indexed tensor bytes drop by `0.997454` while `K≈4.6`, and CPU tensorization latency reduction reaches `0.995549`. This is still not model-forward, GPU, power, or hardware evidence. | Sparse frontier kernel profiling, real memory-traffic accounting, allocator-level branch-overlay measurements, CUDA/model-forward profiling, and matched-accuracy speedups. |
| Calibration and uncertainty are shallow | Branch probabilities matter only if calibrated under distribution shift. | PyBullet shift generalization now reports ECE, Brier, and NLL under held-out mechanism families. The 5-seed aggregate ECE ratio improves to `0.875306` for WPU over baselines, but this is still single-step and accuracy remains mixed. | Multi-step ECE/Brier/NLL, branch collapse tests, temperature/calibration heads, and uncertainty-gated recompute experiments. |
...
1. Close the candidate-oracle gap beyond the current best gap-closure fraction
   of `0.308651` without returning to token processing. Candidate-regret
   targets now improve over aggregate policy selection, but harmful accepts
   remain too frequent. The next step is stronger accept/reject calibration,
   harmful-candidate penalties, and cross-seed perturbation.
...
6. Extend the PyBullet systems profile beyond CPU tensorization latency to model
   forward runtime, CUDA memory, allocator-level memory, and matched-accuracy
   speedups.
