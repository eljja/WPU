...
| Candidate-oracle gap remains open | WPU v2 exposes a useful control surface, but deployed selectors still leave substantial oracle performance unused. | The latest gap audit shows risk-adjusted mechanism routing closes only `0.195451`, `0.244220`, and `0.042131` of the oracle gain at `K=8,16,32`. The decomposition audit shows that this is not caused by omitting one aggregate policy. The no-harm gate audit further shows that margin-only sample-level gates are not sufficient: best closure is `0.082804`, and K=8/16 have negative closure. | Joint retriever-propagator training, calibrated candidate-regret targets, selector uncertainty, no-harm rejection losses, and transfer-stable candidate scoring. |
...
1. Close the candidate-oracle gap beyond the current best gap-closure fraction
   ...
   per-candidate uncertainty, calibrated regret targets, and no-harm rejection
   losses. The no-harm gate audit rejects margin-only thresholding as the
   missing fix.
