...
| Unknown-theory discovery is only a long-term program | Learning relations that expose unknown regularities is a stronger claim than using known object relations. | The object-history hidden-mechanism probe gives a first synthetic positive result: over five seeds, a history scorer trained on `contact_transfer` and `support_transfer` transfers to held-out `hidden_field` with mean relation precision/recall `0.987500` and downstream accuracy `0.992188`. | Simulator-backed held-out-rule or hidden-mechanism benchmarks where learned object relations improve prediction and produce falsifiable new structure. |
