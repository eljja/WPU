...
1. Close the candidate-oracle gap beyond the current conservative gap-closure
   ...
   The next step is joint retriever-propagator training and calibrated
...
2. Improve long-horizon state integrity, not only report it. Simple delta-norm,
   ...
   correction fires, but it does not reduce trigger frequency. The next step is
   a learned stable transition/correction trigger with uncertainty escalation
   and lower correction frequency.
