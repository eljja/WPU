...
| Long-horizon state integrity is not proven | Persistent state is only valuable if delta overlays do not accumulate unrecoverable corruption. | A PyBullet state-integrity audit now tracks constraint validity, bounded delta drift, branch stability, and unsafe-delta rejection. Raw WPU sparse falls to integrity `0.084722` at horizon 25. Guarded state-store projection raises applied-state integrity to `0.958508` for sparse WPU and `0.964322` for local-dense WPU, but raw delta instability remains. A target-relative delta-norm regularized raw rollout only improves sparse H=25 integrity to `0.087153`; unsafe-delta rejection improves sparse integrity to `0.530270` only by rejecting `0.640000` of updates. | Rollout training with rollback, correction, calibration, uncertainty escalation, unsafe-delta rejection, and state-consistency losses. |
...
1. Close the candidate-oracle gap beyond the current conservative gap-closure
   fraction of `0.327146` without returning to token processing.
