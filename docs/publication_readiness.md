...
1. Close the candidate-oracle gap beyond the current conservative gap-closure
   ...
   alone, or post-hoc sparse/dense verification signatures. The next step is
   deeper joint candidate generation, retrieval, propagation verification,
   propagation dynamics, and calibrated accept/reject losses, not another
   post-hoc gate.
