...
| Unknown-theory discovery is only a long-term program | Learning relations that expose unknown regularities is a stronger claim than using known object relations. | Two synthetic positive results now exist. The object-history hidden-mechanism probe transfers relation variables across renamed types with mean relation precision/recall `0.987500`. The local-law probe transfers a history-derived relation selector plus an interpretable inverse-distance law to held-out `hidden_inverse`, reaching delta MSE `0.000828` versus `0.445909` for no relation or type prior. The OOD version shows useful but bounded transfer: far-distance relation recall falls to `0.658594`, and gain/law-form shifts leave residual error even with oracle relations. | Simulator-backed held-out-rule or hidden-mechanism benchmarks where learned object relations improve prediction and produce falsifiable new structure. |
