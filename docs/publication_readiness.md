...
| Long-horizon state integrity is not proven | Persistent state is only valuable if delta overlays do not accumulate unrecoverable corruption. | A PyBullet state-integrity audit now tracks constraint validity, bounded delta drift, branch stability, unsafe-delta rejection, correction, rollback, and dense escalation. Raw WPU sparse falls to integrity `0.084722` at horizon 25. Guarded state-store projection raises applied-state integrity to `0.958508` for sparse WPU and `0.964322` for local-dense WPU, but raw delta instability remains. A target-relative delta-norm regularized raw rollout only improves sparse H=25 integrity to `0.087153`; naive rollout-consistency gives `0.084549`; state-validity and strong state-validity regularization both remain at `0.084722`; unsafe-delta rejection improves sparse integrity to `0.530270` only by rejecting `0.640000` of updates. Rollback raises sparse applied-state integrity to `0.988647`, but with rollback rate `0.812500`. Corrected rollback lowers rollback rate to `0.564167`, but integrity drops to `0.900288`. Escalated corrected rollback raises sparse integrity to `0.914831` and lowers rollback rate to `0.000000`, but uses fallback at rate `0.805833`. Finite-corrected sparse reaches integrity `0.958735` with rollback and escalation both `0.000000`, but correction rate remains high at `0.784166`; this is memory-layer safety, not stable raw sparse dynamics. | Rollout training with learned correction, calibration, uncertainty escalation, lower-correction policies, unsafe-delta rejection, and state-consistency losses. |
...
1. Close the candidate-oracle gap beyond the current conservative gap-closure
   ...
   train-selected deployment, and cross-fit ensemble gating lowers closure.
...
2. Improve long-horizon state integrity, not only report it. Simple delta-norm,
   ...
   insufficient. Finite-corrected projection protects applied state without
   rollback or dense escalation, but correction fires too often, so the next step
   is a learned correction layer with uncertainty escalation and lower
...
5. Improve calibration, not only report it: static uncertainty-gated recompute
   helps only near full local-dense recompute. Add learned low-cost uncertainty
   gates, temperature/calibration heads, and multi-step ECE/Brier/NLL.
