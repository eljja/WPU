...
For the compact thesis, novelty statement, and evidence boundary, see
...
The publishable novelty is the execution model: WPU treats objectified state as
the computational substrate and makes object identity, relation traversal,
causal working-set selection, delta overlays, and branch revision native
operations. That is a creative and testable departure from sequence-only
processing, but it is still a regime hypothesis rather than a universal
superiority result.
...
The v3 world-copy index extension improves the large-`N` story but does not
finish it. `WorldCausalIndex` shows that a hierarchical state substrate can keep
the selected causal slice fixed at `K = 4` while total `N` grows to `10004` in a
controlled non-causal-background probe. That is publishable as substrate
evidence for pre-tensor causal retrieval. It is not evidence that WPU has solved
trained real-world modeling, noisy perception-to-state construction, or
...
| Hierarchical world-copy maintenance is only at substrate level | A real world copy needs streaming updates, region migration, stale-state eviction, and causal-index recall under noisy relations. | `WorldCausalIndex` keeps `K=4` through total `N=10004` with zero non-causal selections in a controlled probe. | Streaming world-copy benchmark with object creation/deletion, region movement, noisy/missing relations, recall/precision of causal slices, and downstream trained propagation accuracy. |
...
2. Improve long-horizon state integrity, not only report it. Simple delta-norm,
   ...
   bounds, target-object delta loss, and stabilized truncated unroll do not
   reduce that bottleneck. The new branch-weighted target-local transition head
   is a small positive step under 5 seeds: H=25 branch accuracy improves from
   `0.612500` to `0.675000` and target-object MSE from `544.710934` to
   `543.834596`, but integrity falls from `0.868758` to `0.850574`. The next
   step is state-validity-aware target-object dynamics, because simple channel
   masking did not preserve the target-head gain.
