...
| Relation repair can add false hypotheses | Repair can recover missing local connectivity, but spurious edges can expand `K` and hurt sparse precision. | The relation-repair probe shows ungated repair recovers frontier recall but drops precision to `0.078994` with near distractors; type-gated repair restores precision to `1.000000` in the controlled case. | Repair precision/recall against simulator relations, downstream loss with and without repair, and learned gates that reject harmful repaired edges. |
