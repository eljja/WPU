...
2. Improve long-horizon state integrity, not only report it. Simple delta-norm,
   ...
   raw-model positive: relation WPU H=25 integrity reaches `0.865848` at bound
   `0.05`, exceeding finite projection (`0.739041`) without correction,
   rollback, rejection, or dense fallback. This improves publication readiness,
   but only as a state-integrity result; trajectory error and branch accuracy
   under bounded updates are still required to rule out under-updating. The next
   step is simulator-resynchronized transition evaluation with bounded deltas,
   state-validity objectives, and adaptive per-feature/per-relation bounds.
