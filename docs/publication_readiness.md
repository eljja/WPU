...
1. Close the candidate-oracle gap beyond the current conservative gap-closure
   ...
   Joint object-set gating is also weaker, so the bottleneck is not merely
   missing candidate-state features. The next step is joint retriever-propagator
   training, candidate generation, and calibrated
...
2. Improve long-horizon state integrity, not only report it. Simple delta-norm,
   ...
   not preserve integrity at low correction frequency. The next step is a
   learned stable transition/correction trigger with uncertainty escalation and
   lower correction frequency, not another hand-coded threshold.
