...
| Objectification quality is not fully solved | WPU performance depends on correct identity, relation, and delta construction. | A PyBullet objectification-quality benchmark now measures identity recall, semantic consistency, relation precision/recall, frontier recall, selected `K`, and `ObjectificationReport` fields. It shows frontier and semantic failures that the scalar contract score can miss. | Add these metrics to the public objectification report, connect them to downstream loss, and evaluate real perception/state adapters. |
...
7. Extend `ObjectificationReport` with frontier completeness and semantic
   identity consistency, then connect objectification-quality metrics to
   downstream propagation loss.
