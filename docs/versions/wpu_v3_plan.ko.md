...
event stream에서 평가하고, conservative stability gate와 bounded correction-policy verifier를
추가한다. `N=8192`, `escape_rate=0.75`에서 verified online은 `noisy_anomaly` objective를
learned `0.266984`에서 `0.193618`로 개선하고, `weak_anomaly`를 `0.334646`에서
`0.202138`로 개선한다. Selected work는 dense `8192`가 아니라 약 `32` 근처로 유지된다.
Paired clean stream에서는 verified online이 learned objective `0.166851`을 `0.159361`로
개선해 hand adaptive `0.154890`에 가까워진다. Verifier가 쓰는 mean top-up budget은 clean
`0.171875`, `noisy_anomaly` `0.0`, `weak_anomaly` `1.09375`다. 이는 WPU-native correction
evidence로 긍정적이지만, 닫힌 해법은 아니다. Verified online은 noisy shift에서 labeled
calibration보다 아직 약하고 clean stream에서는 hand adaptive control보다 약하다. 따라서
다음 기준은 top-up budget을 쓰기 전 marginal correction value를 추정하는 것이다.
