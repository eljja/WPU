...
버전은 conservative stability gate를 추가한다. Calibration은 반복적인 miss 또는 false
probe evidence가 있을 때만 바뀌고, neighbor-support credit은 false-positive evidence가
있을 때만 적용된다. 이는 의도한 world-copy correction loop에 더 가깝다. WPU는 작은
candidate set만 관측하고, 그 관측이 누락된 causal state를 복구했는지 측정한 뒤,
full-world serialization 없이 다음 observation sensitivity를 조정한다. `N=8192`,
`escape_rate=0.75`에서 `noisy_anomaly`는 learned objective `0.261673`에서 online
objective `0.192036`으로 개선되고, recall은 `0.8125`에서 `0.960938`로 오른다.
`weak_anomaly`에서는 objective가 `0.337192`에서 `0.223587`로 개선되고, recall은
`0.373047`에서 `0.763672`로 오른다. Selected work는 약 `32`로 bounded를 유지하며 dense
state copy는 `8192` state unit을 touch한다.

부정적 결과는 좁아졌지만 여전히 남아 있다. Clean stream 전체에서는 conservative gate가
online calibration을 learned-policy objective 근처로 되돌리지만, `N=8192`,
`escape_rate=0.75`에서는 online objective `0.160458`가 learned objective `0.154210`보다
아직 약간 나쁘다. 따라서 남은 경계는 더 엄격한 no-harm online calibration이다. Shifted
correction은 개선됐지만, labeled calibration의 robust 대체물로 주장하려면 더 나은
stability detector가 필요하다.
