...
추가하며, `wpu-verified-online-observation`을 추가한다. 이 mode는 첫 관측이 high-precision
이지만 아직 miss가 남아 있을 때만 최대 2개의 추가 관측을 요청하는 bounded
...
`N=8192`, `escape_rate=0.75`에서 `noisy_anomaly`는 learned objective `0.266984`에서
verified online `0.193618`로 개선되고, recall은 `0.800781`에서 `0.957031`로 오른다.
`weak_anomaly`에서는 verified online이 learned objective `0.334646`을 `0.202138`로
개선하며, unverified online `0.211102`보다 좋고 labeled calibration `0.197684`에
가까워진다. Recall은 `0.390625`에서 `0.822266`으로 오른다. Clean paired stream에서는
verified online이 learned objective `0.166851`을 `0.159361`로 개선해 hand adaptive
`0.154890`에 가까워진다. 이때 mean verifier top-up은 clean `0.171875`,
`noisy_anomaly` `0.0`, `weak_anomaly` `1.09375`로 bounded다. Dense state copy는 여전히
exact지만 `8192` state unit 전체를 touch한다.

남은 경계는 더 선명하다. Verified online은 noisy shift에서 observation budget을 더 많이
써서 labeled calibration보다 objective가 나쁘고, clean stream에서도 hand adaptive control에
도달하지 못한다. 다음 실패는 top-up budget을 쓰기 전에 marginal correction value를 더 잘
추정하는 WPU-native correction-policy verifier다.
