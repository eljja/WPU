...
verified online `0.193756`으로 개선되고, recall은 `0.800781`에서 `0.957031`로 오른다.
`weak_anomaly`에서는 verified online이 learned objective `0.334783`을 `0.202128`로
개선하며, unverified online `0.210699`보다 좋고 labeled calibration `0.197261`에
가까워진다. Recall은 `0.390625`에서 `0.822266`으로 오른다. Clean paired stream에서는
verified online이 learned objective `0.166565`를 `0.159351`로 개선해 hand adaptive
...
소폭 오르며, objective는 online/verified `0.193756`에서 `0.181400`으로 개선된다. 이는
labeled calibration set 없이 작은 labeled-calibration objective `0.180582`에 근접한 결과다.

하지만 이것도 닫힌 해법은 아니다. `weak_anomaly`에서는 실패 원인이 over-observation이 아니라
under-observation이므로 sequential stopping이 도움이 되지 않는다. 이 경우 verified top-up이
`0.202128`로 sequential `0.210699`보다 낫고, labeled calibration `0.197261`이 여전히
가장 좋다. Clean paired stream에서도 sequential stopping은 neutral이고 verified top-up이 가장
좋은 WPU 결과다. 따라서 다음 실패는 composition이다. Token식 full-state recomputation으로
돌아가지 않고, sequential base-budget stopping, verified top-up, 또는 둘의 조합을 언제 쓸지
선택하거나 학습해야 한다.
