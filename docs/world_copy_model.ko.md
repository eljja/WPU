...
`N=8192`, `escape_rate=0.75`에서 `noisy_anomaly`는 learned objective `0.266729`에서
verified online `0.193491`로 개선되고, recall은 `0.800781`에서 `0.957031`로 오른다.
`weak_anomaly`에서는 verified online이 learned objective `0.334783`을 `0.201318`로
개선하며, unverified online `0.208850`보다 좋고 labeled calibration `0.196083`에
가까워진다. Recall은 `0.390625`에서 `0.824219`로 오른다. Clean paired stream에서는
verified online이 learned objective `0.166575`를 `0.159234`로 개선해 hand adaptive
...
objective가 나쁘다. 첫 base-budget value trimming ablation은 negative다. 같은 조건에서
noisy budget은 줄였지만 recall을 잃어 objective가 `0.256521`로 악화되고, `weak_anomaly`도
`0.242431`로 악화된다. 따라서 다음 실패는 naive tail trimming이 아니라 더 안전한
