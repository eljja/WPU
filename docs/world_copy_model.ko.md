...
`N=8192`, `escape_rate=0.75`에서 `noisy_anomaly`는 learned objective `0.266740`에서
verified online `0.193756`으로 개선되고, recall은 `0.800781`에서 `0.957031`로 오른다.
`weak_anomaly`에서는 verified online이 learned objective `0.333978`을 `0.201456`으로
개선하며, unverified online `0.208829`보다 좋고 labeled calibration `0.197006`에
가까워진다. Recall은 `0.390625`에서 `0.824219`로 오른다. Clean paired stream에서는
verified online이 learned objective `0.165791`을 `0.159096`으로 개선해 hand adaptive
`0.154890`에 가까워진다. 이때 mean verifier top-up은 bounded이며 value-gated다. Clean은
top-up `0.171875`와 estimated value `0.006895`, `noisy_anomaly`는 top-up `0.0`,
`weak_anomaly`는 top-up `1.09375`와 estimated value `0.043876`이다. Dense state copy는
여전히 exact지만 `8192` state unit 전체를 touch한다.

남은 경계는 더 선명하다. Value-gated top-up은 weak anomaly와 일부 clean miss에는 유용하지만,
noisy shift는 top-up 이전의 base online budget이 너무 커서 labeled calibration보다
objective가 나쁘다. 따라서 다음 실패는 더 추가할지 여부가 아니라, 더 적은 observation으로
충분한 시점을 추정하는 base-budget value calibration이다.
