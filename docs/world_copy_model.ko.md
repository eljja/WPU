...
추가한다. Calibration은 반복적인 miss 또는 false-probe evidence가 있을 때만 바뀌고,
neighbor-support credit은 false-positive evidence가 있을 때만 적용된다. 이는 의도한
world-copy correction loop에 더 가깝다. WPU는 작은 candidate set만 관측하고, 그 관측이
누락된 causal state를 복구했는지 측정한 뒤, full-world serialization 없이 다음
observation sensitivity를 조정한다. `N=8192`, `escape_rate=0.75`에서 `noisy_anomaly`는
learned objective `0.266602`에서 online objective `0.193756`으로 개선되고, recall은
`0.800781`에서 `0.957031`로 오른다. `weak_anomaly`에서는 objective가 `0.334646`에서
`0.210113`으로 개선되고, recall은 `0.390625`에서 `0.808594`로 오른다. Selected work는 약
`32`로 bounded를 유지하며 dense state copy는 `8192` state unit을 touch한다.

Paired benchmark에서는 이전에 보이던 learned 대비 clean no-harm gap이 사라진다.
`N=8192`, `escape_rate=0.75` clean에서 같은 stream 기준 learned와 online objective는 둘
다 `0.166575`다. 남은 경계는 더 선명하다. Online calibration은 shift 조건에서 labeled
calibration set보다 여전히 약하고, clean stream에서는 hand adaptive rule보다 약하다. Dense
state copy는 `O(N)` work를 쓰지만 여전히 exact다. 따라서 다음 실패는 bounded event-local
execution을 유지하면서 labeled/hand decision에 가까워지는 WPU-native correction-policy
verifier다.
