...
def _priority_state_integrity() -> dict[str, object]:
    ...
    return _row(
        ...
        "Bounded delta parameterization is trajectory-positive, and a branch-weighted target-local transition head gives the first small target-object improvement; next step is larger-seed validation and stronger target-head dynamics.",
    )
...
def _relation_closed_loop_rollout_note() -> str:
    ...
    return (
        ...
        + (
            f" A branch-weighted target-local transition head is the first positive follow-up on this bottleneck: mechanism-target H=25 branch accuracy is "
            f"{float(mechanism_target['rollout_branch_accuracy']):.6f}, trajectory MSE "
            f"{float(mechanism_target['trajectory_mse']):.6f}, and target-object MSE "
            f"{float(mechanism_target['target_object_trajectory_mse']):.6f}, improving over bounded relation WPU branch accuracy "
            f"{float(bounded005['rollout_branch_accuracy']):.6f}, trajectory MSE "
            f"{float(bounded005['trajectory_mse']):.6f}, and target-object MSE "
            f"{float(bounded005['target_object_trajectory_mse']):.6f}. The absolute target-object error is still high, so this is an architectural direction rather than solved dynamics."
            if mechanism_target is not None and bounded005 is not None
            else ""
        )
        + (
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "Bounded delta parameterization은 sparse rollout baseline으로 trajectory-positive까지 확인됐고, branch-weighted target-local transition head가 첫 작은 target-object 개선을 보였다. 다음은 더 큰 seed 검증과 더 강한 target-head dynamics다.",
