...
def _priority_state_integrity() -> dict[str, object]:
    ...
    return _row(
        ...
        "Bounded delta parameterization is trajectory-positive, a branch-weighted target-local transition head gives a small 5-seed target-object improvement at an integrity cost, and simple channel-masked target-head constraints are negative; next step is state-validity-aware transition learning.",
    )
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "Bounded delta parameterization은 sparse rollout baseline으로 trajectory-positive까지 확인됐고, branch-weighted target-local transition head는 작은 target-object 개선을 만들지만 integrity 비용이 있다. 단순 channel-masked constrained target-head는 negative이므로 다음은 state-validity-aware transition learning이다.",
