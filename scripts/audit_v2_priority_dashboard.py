...
def _priority_state_integrity() -> dict[str, object]:
    ...
    return _row(
        ...
        "Bounded delta parameterization is the first raw H=25 integrity positive; next step is simulator-resynchronized trajectory error and branch-accuracy auditing under bounded updates.",
    )
...
def _relation_closed_loop_rollout_note() -> str:
    ...
    return (
        ...
        + baseline_note
        + " This is now a bounded-raw-integrity positive, but trajectory accuracy under small bounds remains unproven."
    )
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "Bounded delta parameterization은 첫 raw H=25 integrity positive다. 다음은 bounded update에서 simulator-resynchronized trajectory error와 branch accuracy가 유지되는지 검증한다.",
