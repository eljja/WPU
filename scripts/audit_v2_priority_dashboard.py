...
def _priority_candidate_oracle_gap() -> dict[str, object]:
    ...
    return _row(
        ...
        source,
        f"Best deployed closure is {best:.6f}; previous aggregate-policy best is {aggregate_best:.6f} and mean aggregate closure is {mean:.6f}.{noharm_note}{regret_note}{penalty_note}{perturb_note}{safety_note}{crossfit_note}{invariant_note}{joint_note}{joint_regression_note}{end_to_end_note}{joint_generator_note}{verified_note}{joint_adapter_note}",
        "Move beyond post-hoc, object-set-only, selector-loss-only, generator-only, verification-feature-only, and shallow branch-logit-adapter probes: jointly train candidate generation, retrieval, propagation dynamics, propagation verification, and calibrated no-harm objectives with held-out seed selection.",
    )
...
def _ko_next_action(priority: int) -> str:
    return {
        1: "Post-hoc gate, object-set-only gate, selector-loss-only gate, generator-only probe, verification-feature-only probe, shallow output-adapter probe를 더 튜닝하기보다 candidate generation, retrieval, propagation verification, propagation dynamics를 joint objective로 묶고, no-harm/calibration target을 held-out seed 전이에 맞게 학습한다.",
        2: "Selective correction으로 수정 범위는 줄였지만 correction trigger rate는 줄이지 못했고, learned trigger도 hard seed split에서 낮은 correction frequency와 높은 integrity를 동시에 만족하지 못했다. 다음은 trigger threshold가 아니라 transition model 자체를 state-validity/correction objective로 안정화하는 것이다.",
