...
def _priority_state_integrity() -> dict[str, object]:
    ...
    return _row(
        ...
        "Bounded delta parameterization is now trajectory-positive as a sparse rollout baseline; next step is adaptive per-feature/per-relation bounds to reduce target-object trajectory error.",
    )
...
def _ko_interpretation(priority: int) -> str:
    interpretations = {
        ...
        2: "Rollback/correction 계열 memory layer는 applied-state safety를 개선하지만 learned dynamics 증거는 아니다. Relation-conditioned rollout에서 raw H=25는 integrity 0.250368, trajectory MSE 6.975125, branch accuracy 0.208333으로 무너진다. Finite projection은 integrity 0.876760까지 올리지만 trajectory MSE 1.695024, branch accuracy 0.250000으로 예측 성능은 약하다. Bounded delta parameterization은 첫 raw-transition positive다. Bound0.05는 correction, rollback, rejection, dense fallback 없이 H=25 integrity 0.870264, trajectory MSE 0.707117, branch accuracy 0.729167을 달성한다. Bound0.1도 integrity 0.793182, trajectory MSE 0.720660, branch accuracy 0.708333으로 positive다. 따라서 under-update 우려는 줄었지만 target-object trajectory MSE 361.358309가 남아 high-fidelity dynamics는 아직 아니다. 다음 P2는 adaptive per-feature/per-relation bound와 target-object trajectory training이다.",
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "Bounded delta parameterization은 sparse rollout baseline으로 trajectory-positive까지 확인됐다. 다음은 target-object trajectory error를 줄이기 위한 adaptive per-feature/per-relation bound다.",
