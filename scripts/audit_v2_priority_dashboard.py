...
def _priority_state_integrity() -> dict[str, object]:
    ...
    return _row(
        ...
        "Bounded delta parameterization is trajectory-positive, but adaptive/split bounds and one-step target loss do not reduce target-object error; next step is unrolled branch/trajectory-consistent training.",
    )
...
def _relation_closed_loop_rollout_note() -> str:
    ...
    return (
        ...
        + (
            ...
            f"{float(targetloss1['target_object_trajectory_mse']):.6f}. This points to unrolled trajectory-consistent training, not another scalar bound or one-step target-loss tweak."
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "Bounded delta parameterization은 sparse rollout baseline으로 trajectory-positive까지 확인됐다. Adaptive/split bounds와 one-step target loss는 부족하므로 다음은 unrolled branch/trajectory-consistent training이다.",
