...
def _priority_state_integrity() -> dict[str, object]:
    ...
    return _row(
        ...
        "Bounded delta parameterization is trajectory-positive, a branch-weighted target-local transition head gives a small 5-seed target-object improvement at an integrity cost, and simple channel-masked target-head constraints plus direct state-validity penalties are negative; next step is a structured transition constraint or correction objective.",
    )
...
def _relation_closed_loop_rollout_note() -> str:
    ...
    bounded005 = find("relation_bounded_delta005", "wpu-cws-indexed-mechanism-relation", 25)
    bounded005_5seed = find("relation_bounded_delta005_5seed", "wpu-cws-indexed-mechanism-relation", 25)
    bounded01 = find("relation_bounded_delta01", "wpu-cws-indexed-mechanism-relation", 25)
    bounded025 = find("relation_bounded_delta025", "wpu-cws-indexed-mechanism-relation", 25)
    bounded05 = find("relation_bounded_delta05", "wpu-cws-indexed-mechanism-relation", 25)
    adaptive025 = find("relation_adaptive_delta025", "wpu-cws-indexed-mechanism-relation", 25)
    split_p010_v005 = find("relation_split_delta_p010_v005", "wpu-cws-indexed-mechanism-relation", 25)
    targetloss1 = find("relation_bounded_delta005_targetloss1", "wpu-cws-indexed-mechanism-relation", 25)
    bounded005_lr3e4_clip1 = find(
        "relation_bounded_delta005_lr3e4_clip1", "wpu-cws-indexed-mechanism-relation", 25
    )
    unrolled_h2_4 = find("relation_bounded_delta005_unrolled_h2_4", "wpu-cws-indexed-mechanism-relation", 25)
    mechanism_target = find(
        "relation_mechanism_target_bounded_delta005", "wpu-cws-indexed-mechanism-target", 25
    )
    mechanism_target_5seed = find(
        "relation_mechanism_target_bounded_delta005_5seed", "wpu-cws-indexed-mechanism-target", 25
    )
    mechanism_target_constrained_5seed = find(
        "relation_mechanism_target_constrained_bounded_delta005_5seed",
        "wpu-cws-indexed-mechanism-target-constrained",
        25,
    )
    mechanism_target_validity001_5seed = find(
        "relation_mechanism_target_validity001_5seed", "wpu-cws-indexed-mechanism-target", 25
    )
    mechanism_target_validity01_5seed = find(
        "relation_mechanism_target_validity01_5seed", "wpu-cws-indexed-mechanism-target", 25
    )
    mechanism_target_validity_5seed = mechanism_target_validity001_5seed or mechanism_target_validity01_5seed
    matched_bounded = bounded005_5seed or bounded005
    matched_target = mechanism_target_5seed or mechanism_target
    delta_norm = find("relation_delta_norm_strong", "wpu-cws-indexed-mechanism-relation", 25)
    scale010 = find("relation_delta_scale010", "wpu-cws-indexed-mechanism-relation", 25)
    scale025 = find("relation_delta_scale025", "wpu-cws-indexed-mechanism-relation", 25)
    stride4 = find("relation_train_stride4", "wpu-cws-indexed-mechanism-relation", 25)
    stride8 = find("relation_train_stride8", "wpu-cws-indexed-mechanism-relation", 25)
    stride4_delta = find("relation_train_stride4_delta1", "wpu-cws-indexed-mechanism-relation", 25)
    stride4_branch_delta = find("relation_train_stride4_branch01_delta1", "wpu-cws-indexed-mechanism-relation", 25)
    multihorizon_w1 = find("relation_multihorizon_w1", "wpu-cws-indexed-mechanism-relation", 25)
    multihorizon_w5 = find("relation_multihorizon_w5", "wpu-cws-indexed-mechanism-relation", 25)
    multihorizon_clip = find("relation_multihorizon_w1_clip1", "wpu-cws-indexed-mechanism-relation", 25)
    ...
    return (
        ...
        + (
            f" Bounded delta parameterization is the first positive raw-model stability result: "
            f"bound0.5 {float(bounded05['state_integrity_score']):.6f}, "
            f"bound0.25 {float(bounded025['state_integrity_score']):.6f}, "
            f"bound0.1 {float(bounded01['state_integrity_score']):.6f}, and "
            f"bound0.05 {float(bounded005['state_integrity_score']):.6f}."
            if bounded005 is not None and bounded01 is not None and bounded025 is not None and bounded05 is not None
            else ""
        )
        + (
            f" Simulator-resynchronized metrics reduce the under-update concern: bound0.05 H=25 trajectory MSE is "
            f"{float(bounded005['trajectory_mse']):.6f} and branch accuracy is "
            f"{float(bounded005['rollout_branch_accuracy']):.6f}, versus finite projection trajectory MSE "
            f"{float(finite['trajectory_mse']):.6f} and branch accuracy "
            f"{float(finite['rollout_branch_accuracy']):.6f}. Target-object trajectory MSE remains high at "
            f"{float(bounded005['target_object_trajectory_mse']):.6f}, so high-fidelity dynamics are not solved."
            if bounded005 is not None
            and bounded005.get("trajectory_mse") not in {None, ""}
            and finite.get("trajectory_mse") not in {None, ""}
            else ""
        )
        + (
            f" Follow-up target-object fixes are negative or mixed: learned adaptive bounds reach branch accuracy "
            f"{float(adaptive025['rollout_branch_accuracy']):.6f} with target-object MSE "
            f"{float(adaptive025['target_object_trajectory_mse']):.6f}; split position/velocity bounds reach branch accuracy "
            f"{float(split_p010_v005['rollout_branch_accuracy']):.6f} with target-object MSE "
            f"{float(split_p010_v005['target_object_trajectory_mse']):.6f}; target-object loss weight 1.0 reaches branch accuracy "
            f"{float(targetloss1['rollout_branch_accuracy']):.6f} with target-object MSE "
            f"{float(targetloss1['target_object_trajectory_mse']):.6f}. This rules out another scalar bound or one-step target-loss tweak as the missing fix."
            if adaptive025 is not None and split_p010_v005 is not None and targetloss1 is not None
            else ""
        )
        + (
            f" Unrolled trajectory-consistent training has now been tested as a follow-up: full recurrent gradient was non-finite in the initial probe, while the stabilized truncated H=2/4 run reaches H=25 branch accuracy "
            f"{float(unrolled_h2_4['rollout_branch_accuracy']):.6f} and target-object MSE "
            f"{float(unrolled_h2_4['target_object_trajectory_mse']):.6f}, essentially matching the same-lr bounded-only baseline branch accuracy "
            f"{float(bounded005_lr3e4_clip1['rollout_branch_accuracy']):.6f} and target-object MSE "
            f"{float(bounded005_lr3e4_clip1['target_object_trajectory_mse']):.6f}. This shows unrolled-loss-only training is insufficient."
            if unrolled_h2_4 is not None and bounded005_lr3e4_clip1 is not None
            else ""
        )
        + (
            f" A branch-weighted target-local transition head is the first positive follow-up on this bottleneck. In the preferred matched-seed H=25 comparison, mechanism-target branch accuracy is "
            f"{float(matched_target['rollout_branch_accuracy']):.6f}, trajectory MSE "
            f"{float(matched_target['trajectory_mse']):.6f}, and target-object MSE "
            f"{float(matched_target['target_object_trajectory_mse']):.6f}, improving over bounded relation WPU branch accuracy "
            f"{float(matched_bounded['rollout_branch_accuracy']):.6f}, trajectory MSE "
            f"{float(matched_bounded['trajectory_mse']):.6f}, and target-object MSE "
            f"{float(matched_bounded['target_object_trajectory_mse']):.6f}. The absolute target-object error is still high, so this is an architectural direction rather than solved dynamics."
            if matched_target is not None and matched_bounded is not None
            else ""
        )
        + (
            f" A constrained target-head variant that masks the residual to position/velocity channels is negative as a standalone fix: H=25 branch accuracy "
            f"{float(mechanism_target_constrained_5seed['rollout_branch_accuracy']):.6f}, trajectory MSE "
            f"{float(mechanism_target_constrained_5seed['trajectory_mse']):.6f}, target-object MSE "
            f"{float(mechanism_target_constrained_5seed['target_object_trajectory_mse']):.6f}, and integrity "
            f"{float(mechanism_target_constrained_5seed['state_integrity_score']):.6f}. This shows channel masking alone does not preserve the target-head gain; P2 needs state-validity-aware transition learning."
            if mechanism_target_constrained_5seed is not None
            else ""
        )
        + (
            f" A direct state-validity penalty on the mechanism-target model is also negative as a standalone dynamics fix: H=25 branch accuracy rises to "
            f"{float(mechanism_target_validity_5seed['rollout_branch_accuracy']):.6f}, but trajectory MSE jumps to "
            f"{float(mechanism_target_validity_5seed['trajectory_mse']):.6f} and target-object MSE to "
            f"{float(mechanism_target_validity_5seed['target_object_trajectory_mse']):.6f}. This means the next P2 step needs structured transition constraints or correction objectives, not another scalar validity penalty."
            if mechanism_target_validity_5seed is not None
            else ""
        )
        + (
        ...
        + (
            f" Temporal delta scaling is also negative: scale0.25 {float(scale025['state_integrity_score']):.6f} "
            f"and scale0.10 {float(scale010['state_integrity_score']):.6f}."
            if scale010 is not None and scale025 is not None
            else ""
        )
        + (
            f" Short-stride simulator targets are negative: stride4 {float(stride4['state_integrity_score']):.6f}, "
            f"stride8 {float(stride8['state_integrity_score']):.6f}, stride4-delta-only "
            f"{float(stride4_delta['state_integrity_score']):.6f}, and stride4-branch0.1-delta1 "
            f"{float(stride4_branch_delta['state_integrity_score']):.6f}."
            if stride4 is not None
            and stride8 is not None
            and stride4_delta is not None
            and stride4_branch_delta is not None
            else ""
        )
        + (
            f" Explicit multi-horizon simulator targets are also negative: w1 "
            f"{float(multihorizon_w1['state_integrity_score']):.6f}, w5 "
            f"{float(multihorizon_w5['state_integrity_score']):.6f}, and w1+gradclip "
            f"{float(multihorizon_clip['state_integrity_score']):.6f}."
            if multihorizon_w1 is not None and multihorizon_w5 is not None and multihorizon_clip is not None
            else ""
        )
        + baseline_note
        + " This is now a bounded-raw-integrity and trajectory-positive sparse rollout baseline, not yet a high-fidelity dynamics model."
    )
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "Bounded delta parameterization은 sparse rollout baseline으로 trajectory-positive까지 확인됐고, branch-weighted target-local transition head는 작은 target-object 개선을 만들지만 integrity 비용이 있다. 단순 channel-masked constrained target-head와 직접 state-validity penalty는 negative이므로 다음은 structured transition constraint 또는 correction objective다.",
