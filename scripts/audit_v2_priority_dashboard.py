...
def _priority_state_integrity() -> dict[str, object]:
    ...
    return _row(
        ...
        "Bounded delta parameterization is trajectory-positive, and a branch-weighted target-local transition head gives a small 5-seed target-object improvement at an integrity cost; next step is constrained target-head dynamics that preserve state integrity.",
    )
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "Bounded delta parameterization은 sparse rollout baseline으로 trajectory-positive까지 확인됐고, branch-weighted target-local transition head가 5-seed에서 작은 target-object 개선을 보였지만 integrity 비용이 있다. 다음은 state integrity를 보존하는 constrained target-head dynamics다.",
