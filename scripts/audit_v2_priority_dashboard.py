...
def _priority_state_integrity() -> dict[str, object]:
    ...
    return _row(
        ...
        "Bounded delta parameterization is trajectory-positive, but adaptive/split bounds, one-step target loss, and stabilized truncated unrolled loss do not reduce target-object error; next step is branch-conditioned local transition dynamics and target-object transition-head expressivity.",
    )
...
def _relation_closed_loop_rollout_note() -> str:
    ...
    return (
        ...
        + (
            ...
            f"{float(bounded005_lr3e4_clip1['target_object_trajectory_mse']):.6f}. The next fix must change transition-head/local dynamics expressivity, not only add unrolled loss."
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "Bounded delta parameterization은 sparse rollout baseline으로 trajectory-positive까지 확인됐다. Adaptive/split bounds, one-step target loss, truncated unroll은 부족하므로 다음은 branch-conditioned local transition dynamics와 target-object transition head 표현력이다.",
