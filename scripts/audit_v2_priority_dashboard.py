...
def _priority_simulator_grounding() -> dict[str, object]:
    ...
    return _row(
        ...
        "Upgrade the N_bg=512 micro-screen to medium/full training, add mechanisms, and add long-horizon simulator rollouts.",
    )
...
def _ko_interpretation(priority: int) -> str:
    return {
        ...
        3: "PyBullet benchmark는 7개 seed와 background N_bg=128까지 본훈련 baseline-complete로 확장됐고, N_bg=256 screen은 total N=261에서 WPU/graph/token baseline을 모두 완료했다. Medium-training N_bg=256 baseline-complete run은 evidence quality를 올린다. Best WPU인 wpu-cws-indexed-local-dense는 accuracy 0.466667이고 best baseline인 graph-transformer는 0.450000이며, best WPU는 해당 best-accuracy baseline보다 forward latency 기준 60.629526x 빠르다. 단, margin이 작고 단일 cup family이므로 broad superiority claim은 아니다. 새 N_bg=512 baseline-complete micro-screen은 total N=517에서 WPU/graph/token baseline을 모두 포함한다. Best WPU wpu-cws-indexed-sparse는 accuracy 0.375000이고 best baseline은 0.333333이며, best WPU는 해당 best-accuracy baseline보다 forward latency 기준 75.471861x 빠르다. 그러나 이 run은 3 seeds, 2 training steps, 8 samples의 저예산 micro-screen이므로 large-N coverage evidence로만 해석한다. Coverage audit는 10개 PyBullet 축을 추적하고, 별도의 WPU-only large-state extension도 N_bg=512, total N=517까지 실행됐지만 dense graph baseline이 같은 higher-budget protocol에서 완료되지 않았으므로 systems feasibility evidence로만 취급한다. Simulator-backed evidence는 강화됐지만 mechanism 다양성, higher-budget baseline-complete large-N comparison, perception/state adapter가 아직 부족하다.",
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        3: "N_bg=512 micro-screen을 medium/full training baseline-complete run으로 승격하고, 더 많은 mechanism, long-horizon simulator rollout, perception/state adapter를 추가한다.",
