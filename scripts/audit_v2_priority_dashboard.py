...
def _priority_calibration() -> dict[str, object]:
    ...
    return _row(
        ...
        "Move from hand-threshold recompute to learned low-cost uncertainty gates, branch calibration loss, and multi-step ECE/Brier/NLL.",
    )
...
def _ko_interpretation(priority: int) -> str:
    return {
        ...
        2: "Rollback-only memory layer는 sparse WPU H=25 integrity를 0.988647까지 올리지만 rollback rate가 0.812500으로 매우 높다. Corrected rollback은 rollback rate를 0.564167까지 낮추지만 integrity가 0.900288로 떨어진다. Escalated corrected rollback은 local-dense fallback을 사용해 integrity를 0.914831로 올리고 rollback rate를 0.000000으로 낮춘다. 새 finite-corrected run은 finite-safe delta clipping과 correction-only projection으로 integrity 0.958735, rollback rate 0.000000, escalation rate 0.000000을 달성하지만 correction rate가 0.784166으로 높다. 따라서 P2는 memory-layer safety가 유효함을 보이지만, raw delta stability가 해결된 것은 아니다.",
...
def _ko_next_action(priority: int) -> str:
    return {
        ...
        2: "단순 delta-norm, rollout-consistency, state-validity regularization은 부족하다. Guarded/corrected state-store projection은 유지하되 correction rate를 낮추는 learned correction과 uncertainty escalation을 모델-메모리 계층에 넣는다.",
        ...
        5: "Hand-threshold recompute가 아니라 학습 가능한 저비용 uncertainty gate, branch calibration loss, multi-step ECE/Brier/NLL을 추가한다.",
