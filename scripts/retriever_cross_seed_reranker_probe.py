...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate leave-one-seed-out WPU retrieval reranker generalization.")
    ...
    parser.add_argument("--out", type=Path, default=Path("docs/experiments/wpu_v2_retriever_cross_seed_reranker.csv"))
...
def _run_group(background_objects: int, causal_obstacles: int, args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    for seed in args.seeds:
        print(f"cross-seed collect seed={seed} N={total_n} K={causal_k}", flush=True)
    ...
    for heldout_seed in args.seeds:
        ...
        reranker = _train_reranker(train_examples, args, len(candidate_names))
        condition_rows = _summarize_cross_seed(test_examples, train_examples, reranker, candidate_names, args)
...
def _summarize_cross_seed(
    ...
    rows = [
        _policy_row(
            test_examples,
            policy="cross_seed_static_base_choice",
            selected_modes=[static_mode] * len(test_examples),
            candidate_names=candidate_names,
        ),
        _policy_row(
            test_examples,
            policy="cross_seed_static_generated_choice",
            selected_modes=[generated_static_mode] * len(test_examples),
            candidate_names=candidate_names,
        ),
        _policy_row(
            test_examples,
            policy="cross_seed_generated_reranker",
            selected_modes=reranker_modes,
            candidate_names=candidate_names,
        ),
        _policy_row(
            test_examples,
            policy="cross_seed_margin_safe_reranker",
