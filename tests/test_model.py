...
def test_mechanism_conditioned_route_uses_physics_context_without_dense_compute() -> None:
    dataset = WorkingSetPhysicsDataset(size=2, seed=9, background_objects=32, causal_obstacles=8, interaction_mode="pairwise")
    batch, target_delta, labels, _ = collate_working_set_samples([dataset[0], dataset[1]])
    model = create_model(
    ...
    assert model.last_working_set_stats is not None
    assert model.last_working_set_stats.dense_compute_ratio == 0.0
    assert model.mechanism_context_encoder[-1].weight.grad is not None
...
def test_mechanism_factorized_route_uses_scale_shift_without_dense_compute() -> None:
    dataset = WorkingSetPhysicsDataset(size=2, seed=9, background_objects=32, causal_obstacles=8, interaction_mode="pairwise")
    batch, target_delta, labels, _ = collate_working_set_samples([dataset[0], dataset[1]])
    model = create_model(
    ...
    assert model.last_working_set_stats is not None
    assert model.last_working_set_stats.dense_compute_ratio == 0.0
    assert model.mechanism_factor_gate[-1].weight.grad is not None
    assert model.mechanism_factor_gate[-1].weight.grad.norm().item() > 0.0


def test_mechanism_branch_route_conditions_branch_logits_without_dense_compute() -> None:
    dataset = WorkingSetPhysicsDataset(size=2, seed=9, background_objects=32, causal_obstacles=8, interaction_mode="pairwise")
    batch, target_delta, labels, _ = collate_working_set_samples([dataset[0], dataset[1]])
    model = create_model(
    ...
    assert model.mechanism_factor_gate[-1].weight.grad is not None
    assert model.mechanism_factor_gate[-1].weight.grad.norm().item() > 0.0
    assert model.mechanism_branch_head[-1].weight.grad is not None
    assert model.mechanism_branch_head[-1].weight.grad.norm().item() > 0.0


def test_mechanism_branch_expert_route_uses_branch_specific_logits_without_dense_compute() -> None:
    dataset = WorkingSetPhysicsDataset(size=2, seed=9, background_objects=32, causal_obstacles=8, interaction_mode="pairwise")
    batch, target_delta, labels, _ = collate_working_set_samples([dataset[0], dataset[1]])
    model = create_model(
    ...
    assert model.mechanism_factor_gate[-1].weight.grad is not None
    assert model.mechanism_factor_gate[-1].weight.grad.norm().item() > 0.0
    assert model.mechanism_branch_queries.grad is not None
    assert model.mechanism_branch_queries.grad.norm().item() > 0.0
    assert model.mechanism_branch_expert_head[-1].weight.grad is not None
    assert model.mechanism_branch_expert_head[-1].weight.grad.norm().item() > 0.0


def test_mechanism_relation_route_uses_relation_messages_without_dense_compute() -> None:
    dataset = WorkingSetPhysicsDataset(size=2, seed=9, background_objects=32, causal_obstacles=8, interaction_mode="pairwise")
    batch, target_delta, labels, _ = collate_working_set_samples([dataset[0], dataset[1]])
    model = create_model(
    ...
    assert model.mechanism_factor_gate[-1].weight.grad is not None
    assert model.mechanism_factor_gate[-1].weight.grad.norm().item() > 0.0
    assert model.mechanism_relation_message[-1].weight.grad is not None
    assert model.mechanism_relation_message[-1].weight.grad.norm().item() > 0.0
    assert model.mechanism_branch_head[-1].weight.grad is not None
    assert model.mechanism_branch_head[-1].weight.grad.norm().item() > 0.0


def test_mechanism_target_route_uses_target_delta_expert_without_dense_compute() -> None:
    dataset = WorkingSetPhysicsDataset(size=2, seed=9, background_objects=32, causal_obstacles=8, interaction_mode="pairwise")
    batch, target_delta, labels, _ = collate_working_set_samples([dataset[0], dataset[1]])
    model = create_model(
    ...
    assert model.mechanism_relation_message[-1].weight.grad is not None
    assert model.mechanism_relation_message[-1].weight.grad.norm().item() > 0.0
    assert model.mechanism_target_delta_head[-1].weight.grad is not None
