...
def test_world_copy_baseline_comparison_probe_runs(tmp_path: Path) -> None:
    output = tmp_path / "baseline_comparison.csv"
    report = tmp_path / "baseline_comparison.md"
    report_ko = tmp_path / "baseline_comparison.ko.md"
    result = subprocess.run(
        [
            sys.executable,
            "scripts/world_copy_baseline_comparison_probe.py",
            "--world-sizes",
            "64",
            "--k-values",
            "4",
    ...
    assert report_ko.exists()
    text = output.read_text(encoding="utf-8")
    assert "wpu-hybrid" in text
    assert "serialized-token" in text
...
def test_world_copy_adaptive_observation_budget_probe_runs(tmp_path: Path) -> None:
    output = tmp_path / "adaptive_observation_budget.csv"
    result = subprocess.run(
        [sys.executable, "scripts/world_copy_adaptive_observation_budget_probe.py",
         "--world-sizes", "64", "--escape-rate", "0", "0.5",
    ...
    assert result.returncode == 0, result.stderr
    text = output.read_text(encoding="utf-8")
    assert "wpu-adaptive-observation" in text
    assert "mean_observation_budget" in text
...
def test_world_copy_calibrated_observation_policy_probe_runs(tmp_path: Path) -> None:
    output = tmp_path / "calibrated_observation_policy.csv"
    result = subprocess.run(
        [sys.executable, "scripts/world_copy_calibrated_observation_policy_probe.py",
         "--world-sizes", "64", "--escape-rate", "0", "0.5",
         "--eval-shifts", "clean", "weak_anomaly",
         "--train-samples", "24", "--calibration-samples", "8",
         "--eval-samples", "4", "--train-steps", "8",
         "--fixed-budget", "4", "--max-budget", "4", "--contamination", "8",
         "--horizon", "3", "--k-ref", "4",
         "--out-csv", str(output), "--out-md", str(tmp_path / "report.md"),
         "--out-ko-md", str(tmp_path / "report.ko.md")],
    ...
    assert result.returncode == 0, result.stderr
    text = output.read_text(encoding="utf-8")
    assert "wpu-calibrated-learned-observation" in text
    assert "calibration_scale" in text
    assert "objective" in text


def test_world_copy_online_calibration_policy_probe_runs(tmp_path: Path) -> None:
    output = tmp_path / "online_calibration_policy.csv"
    result = subprocess.run(
        [sys.executable, "scripts/world_copy_online_calibration_policy_probe.py",
         "--world-sizes", "64", "--escape-rate", "0", "0.5",
         "--eval-shifts", "clean", "weak_anomaly",
         "--train-samples", "24", "--calibration-samples", "8",
         "--streams", "4", "--train-steps", "8",
         "--fixed-budget", "4", "--max-budget", "4", "--contamination", "8",
         "--horizon", "3", "--k-ref", "4",
         "--out-csv", str(output), "--out-md", str(tmp_path / "report.md"),
         "--out-ko-md", str(tmp_path / "report.ko.md")],
    ...
    assert "wpu-composed-online-observation" in text
    assert "wpu-learned-composed-online-observation" in text
